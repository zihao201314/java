...
public class GetXml {
	...
	public static Map getXml(String pathAndName) {
		...
		List<Element> EltsBean0 = null;
		try {
			EltsBean0 = XPath.selectNodes(root, "entry");
		} catch (JDOMException e1) {
			// TODO 自动生成的 catch 块
			e1.printStackTrace();
		}
		
		for (Element bean0 : EltsBean0) {
			
			String key = bean0.getAttributeValue("key");
			String value = bean0.getAttributeValue("value");
			
			if("constraints#xy_size".equals(key)) {
				Map ViewLayoutSize=new HashMap();
				ViewLayoutSize.put("size", value);
				all.put("size", ViewLayoutSize);
				System.out.println();
			}
...
//					取出entry定义里的值
			...
			} else if (!"Group".equals(featureFirst) && !"Composite".equals(featureFirst)) {

				boxNumber++;
				Map nullBox = new HashMap<String, Map>();
//			获取所有第二层的bean
...
//				取出当前第二层bean的所有entry	
				List<Element> Elts2 = null;
				try {
					Elts2 = XPath.selectNodes(bean1, "entry");
				} catch (JDOMException e) {
					System.out.println("根节点下没有entry");
				}
//				循环entry
				for (Element elt2 : Elts2) {
//					取出entry定义里的值
					String key = elt2.getAttributeValue("key");
					String value = elt2.getAttributeValue("value");
					nullBox.put(key, value);
				}
				all.put("nullBox_" + featureFirst + boxNumber, nullBox);
			}
